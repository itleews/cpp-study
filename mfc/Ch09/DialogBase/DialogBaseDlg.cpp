...
void CDialogBaseDlg::OnBnClickedStart()
{
	...
	if (m_num1 >= m_num2) {
		MessageBox(_T("숫자1이 숫자2보다 작아야 합니다."), _T("오류!"), MB_ICONERROR);
		return;
	}
	...
	std::set<int> uniqueNumbers; // 중복을 허용하지 않는 set 사용

	while (uniqueNumbers.size() < m_count) {
		int num = 1 + rand() % (m_num2 - m_num1 + 1);
		uniqueNumbers.insert(num); // 중복이 자동으로 걸러짐
	}

	// set에 저장된 값들을 리스트 박스에 추가한다.
