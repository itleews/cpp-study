...
CSemaphoreDlg::CSemaphoreDlg(CWnd* pParent /*=nullptr*/)
...
BEGIN_MESSAGE_MAP(CSemaphoreDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_START, &CSemaphoreDlg::OnBnClickedStart)
END_MESSAGE_MAP()


// CSemaphoreDlg 메시지 처리기
...
UINT WorkThread(LPVOID pParam)
{
	...
	while (1) {
		srand(rand() + ::GetCurrentThreadId() + (unsigned)time(NULL));
		::Sleep((rand() + ::GetCurrentThreadId() + (unsigned)time(NULL)) % 2000);
		g_sem.Lock();
		ptr->SetPos(0);
		for (int i = 0; i < 100; i++) {
			ptr->SetPos(i);
...
void CSemaphoreDlg::OnBnClickedStart()
{
